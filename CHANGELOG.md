...
- 2026-08-13 — Changed the scripts to exit non-zero on failures: write scripts exit `1` when any repo ends in a `FAIL` row, `github-find-repos-with-homepage` exits `1` when any homepage is `BROKEN`, and `github-scan-secrets` exits `1` on scan failures and `2` on findings — previously every script exited `0`, so chained workflows and CI could not detect problems.
- 2026-08-13 — Hardened `github-scan-secrets`: grep matches are redacted (long token-like strings become `[REDACTED]`), the results directory is `chmod 700`, clones authenticate with the gh token (private repos work) and never prompt for credentials, leaks are counted with `jq length`, and gitleaks runs via the `git` subcommand where available (`detect` is deprecated since gitleaks 8.19).
- 2026-08-13 — Hardened `DRY_RUN` in all write scripts: every value except an explicit off (`0`, `false`, `no`, `off`, empty) now enables the dry run — previously `DRY_RUN=true` silently ran the destructive action for real — and each write script also accepts `--dry-run`.
- 2026-08-13 — Resolved short-flag collisions: `--node-only` of `github-find-repos-with-license` moved from `-n` to `-N` (`-n` stays `--no-check`) and `--check-pages` of `github-find-repos-with-only-readme` moved from `-p` to `-P` (`-p` stays the Pages source path of `github-enable-pages`); every short letter now has one meaning repo-wide.
- 2026-08-13 — Fixed `github-delete-pages-branch` treating an unknown Pages source as `gh-pages`: repositories whose Pages are built by GitHub Actions (`build_type=workflow`) or whose source branch cannot be determined are now refused instead of getting their `gh-pages` branch deleted.
- 2026-08-13 — Renamed `github-disable-pages` to `github-delete-pages-branch`: the old name suggested a symmetric undo of `github-enable-pages`, while the script permanently deletes the `gh-pages` branch; a red warning banner now precedes every real (non-dry-run) run.
- 2026-08-13 — Extracted the code shared by all fifteen scripts into `shared/__shared.sh` and `shared/__colors.sh` (colors, status painting, row printing, badges, slug parsing, usage extraction, visibility validation, input source resolution, the gh-token git credential helper) — about a third of the codebase; header labels are now aligned via `print_kv`.
- 2026-08-13 — Fixed `github-find-repos-by-metadata` matching the query against whole rows instead of the value column: `-q piecioshka` matched every repo (slug column) and `-q description` matched every repo with any description (field-name column); the query now only matches metadata values.
